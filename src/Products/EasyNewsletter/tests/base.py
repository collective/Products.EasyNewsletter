...
def parsed_payloads_from_msg(msg):
    ...
    parsed_payloads['to'] = [safe_unicode(h[0].strip()) for h in decode_header(parsed_msg.get('To'))]
    parsed_payloads['from'] = [safe_unicode(h[0].strip()) for h in decode_header(parsed_msg.get('From'))]
